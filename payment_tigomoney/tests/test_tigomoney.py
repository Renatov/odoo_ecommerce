...
import urlparse
class TigomoneyCommon(PaymentAcquirerCommon):
    def setUp(self):
        super(TigomoneyCommon, self).setUp()
        self.tigomoney = self.env.ref('payment.payment_acquirer_tigomoney')
        # some CC
...
class TigomoneyForm(TigomoneyCommon):

    def test_10_tigomoney_form_render(self):
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        # be sure not to do stupid things
        self.tigomoney.write({'Numeor de documento': 'tde+tigomoney-facilitator@odoo.com', 'fees_active': False})
        self.assertEqual(self.tigomoney.environment, 'test', 'test without test environment')
        ...
        # render the button
        res = self.tigomoney.render(
            'test_ref0', 0.01, self.currency_euro.id,
            values=self.buyer_values)

        form_values = {
            'cmd': '_xclick',
            'business': 'tde+tigomoney-facilitator@odoo.com',
            ...
            'return': '%s' % urlparse.urljoin(base_url, TigomoneyController._return_url),
            'notify_url': '%s' % urlparse.urljoin(base_url, TigomoneyController._notify_url),
            'cancel_return': '%s' % urlparse.urljoin(base_url, TigomoneyController._cancel_url),
        }
    ...
    def test_11_tigomoney_form_with_fees(self):
        # be sure not to do stupid things
    ...
    def test_20_tigomoney_form_management(self):
        # be sure not to do stupid things
        self.assertEqual(self.paypal.environment, 'test', 'test without test environment')
        ...
        with self.assertRaises(ValidationError):
            self.env['payment.transaction'].form_feedback(tigomoney_post_data, 'paypal')
        ...
        # validate it
        tx.form_feedback(tigomoney_post_data, 'tigomoney')
        # check
